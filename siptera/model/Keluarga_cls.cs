...
namespace siptera.model
{
    ...
    internal class Keluarga_cls
    {
        ...
        public DataTable tampilAnggotaKeluarga(String id_keluarga)
        {
            string Query = @"
        SELECT p.nama, ak.hubungan_dalam_keluarga
        FROM keluarga k
        JOIN anggota_keluarga ak ON k.id_keluarga = ak.id_keluarga
        JOIN penduduk p ON ak.id_penduduk = p.id_penduduk
        WHERE k.no_kk = '" + id_keluarga + "'";

            return server.eksekusiQuery(Query);
        }


        public DataTable tampilByAlamat(String alamat)
        {
            Query = "select * from keluarga where alamat like '" + alamat + "%'";
            return server.eksekusiQuery(Query);
        }
